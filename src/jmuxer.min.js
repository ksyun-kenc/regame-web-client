    ...
    */
    class ExpGolomb {
      ...
      readBits(size, moveIndex = true) {
        ...
        const result = this.getBits(size, this.index, moveIndex); // console.log(`    read bits: result=${result}`);
    ...
    class H264Parser {
      ...
      static parseHeader(unit) {
        let decoder = new ExpGolomb(unit.getPayload()); // skip NALu type
        ...
        if (!this.initialized) {
          ...
          for (let type of this.trackTypes) {
            ...
            if (pay && pay.byteLength) {
              ...
              log(`put segment (${type}): dts: ${track.dts} frames: ${track.mp4track.samples.length} second: ${duration}`);
              track.flush();
              this.seq++;
            }
          }
    ...
    class BufferController extends Event {
      ...
      initCleanup(cleanMaxLimit) {
        try {
          ...
          error(`Error occured while cleaning ${this.type} buffer - ${e.name}: ${e.message}`);
        }
      }
    ...
    class JMuxer extends Event {
      ...
      onBuffer(data) {
        if (this.options.readFpsFromTrack && typeof data.fps !== 'undefined' && this.options.fps != data.fps) {
          ...
          log(`JMuxer changed FPS to ${data.fps} from track data`);
        }
      ...
      onBufferError(data) {
        if (data.name == 'QuotaExceeded') {
          log(`JMuxer cleaning ${data.type} buffer due to QuotaExceeded error`);
